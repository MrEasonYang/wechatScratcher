...
class Wechat
{
	...
	const SET_OAUTH_URL='https://open.weixin.qq.com/connect/oauth2/authorize?';
...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 * @return array $openid
	 */
	public function getOpenIdFromOAuth() {
		if (!$this->access_token && !$this->checkAuth()) return false;
		$url = self::SET_OAUTH_URL.'appid='.$this->appid.'&redirect_uri='.self::OAUTH_RETURN_URL.'&response_type=code&scope=snsapi_base&state=1#wechat_redirect';
